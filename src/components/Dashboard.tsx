...
export function Dashboard() {
    ...
    useEffect(() => {
        // If entries are already loaded, show content immediately
        if (entries.length > 0) {
            setIsReady(true);
            return;
        }
        // Otherwise, defer heavy rendering to allow page transition to finish smoothly
    ...
    return (
        <div className="relative min-h-screen pb-32 text-foreground overflow-x-hidden">
            ...
            >
                ...
                {!isReady ? (
                    // Lightweight loading state while transition finishes
                    <div className="space-y-8 opacity-60 animate-pulse">
                        <div className="h-[200px] bg-muted/20 rounded-3xl" />
                        <div className="h-[300px] bg-muted/20 rounded-3xl" />
                    </div>
                ) : (
                    <>
                        {/* Chart Section */}
